...
AttachAppointmentTabView.prototype.showMe = function(){
	...
	var listHeaders = [];
    listHeaders.push(new DwtListHeaderItem({field:"subject",text:biz_vnc_crm_client.subject,align:DwtLabel.ALIGN_LEFT,icon:"Zimbra"}));
    listHeaders.push(new DwtListHeaderItem({field:"calendar",text:biz_vnc_crm_client.calendar,align:DwtLabel.ALIGN_LEFT}));
    listHeaders.push(new DwtListHeaderItem({field:"start_date",text:biz_vnc_crm_client.start_date,align:DwtLabel.ALIGN_LEFT}))
	listHeaders.push(new DwtListHeaderItem({field:"itemid",text:biz_vnc_crm_client.start_date,align:DwtLabel.ALIGN_LEFT,visible:false}))

	var listParams = {parent:this,headerList:listHeaders,controller:new ZmListController(this,appCtxt.getCurrentApp())};
...
AttachAppointmentTabView.prototype.search_appt = function() {
	...
	response = appCtxt.getCalManager()._search({start:start_date.getTime(),end:end_date.getTime(),offset:0,query:query});
	var searchResultVector = new AjxVector();
	for (var i=0;i<response.size();i++){
		console.dir(response.get(i));
		searchResultVector.add({"subject":response.get(i).getName(),"calendar":appCtxt.getFolderTree().getById(response.get(i).getFolderId()).getName(),"start_date":new AjxDateFormat(AjxDateFormat.SHORT).format(new Date(response.get(i).getStartTime())),"itemid":response.get(i).invId});
	}
