...
ZaCRMCountryModel.addPerson  = function () {
	if(this.parent.addCountryDlg) {
		...
		if(flag == 0)
		{
			...
			var j = JSON.stringify({action:"ADD",object:"country",countryId:obj[ZaCRMadmin.A_countryId],countryName:obj[ZaCRMadmin.A_countryName],countryCode:obj[ZaCRMadmin.A_countryCode],status:obj[ZaCRMadmin.A_countryStatus],createBy:obj[ZaCRMadmin.A_countryCreatedby],writeBy:obj[ZaCRMadmin.A_countryWriteby]});
			var json = "jsonobj=" + j;
			var reqHeader = {"Content-Type":"application/x-www-form-urlencoded"};
			var reqJson = AjxStringUtil.urlEncode(json);
			var response = AjxRpc.invoke(reqJson,biz_vnc_crm_admin.jspUrl, reqHeader, null, false);
