...
ZaCRMChannelModel.addPerson  = function () {
	if(this.parent.addchannelDlg) {
		...
		if(flag == 0)
		{
			...
			var j = JSON.stringify({action:"ADD",object:"channel",channelId:obj[ZaCRMadmin.A_channelId],channelName:obj[ZaCRMadmin.A_channelName],status:obj[ZaCRMadmin.A_channelStatus],createBy:obj[ZaCRMadmin.A_channelCreatedby],writeBy:obj[ZaCRMadmin.A_channelWriteby]});
			var json = "jsonobj=" + j;
			var reqHeader = {"Content-Type":"application/x-www-form-urlencoded"};
			var reqJson = AjxStringUtil.urlEncode(json);
			var response = AjxRpc.invoke(reqJson,biz_vnc_crm_admin.jspUrl, reqHeader, null, false);
